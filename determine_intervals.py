...
#constraint = [["x1",">=","-5.12"],["x2",">=","-5.12"],["x3",">=","-5.12"],["x1","<=","5.12"],["x2","<=","5.12"],["x3","<=","5.12"]]
#constraint = [["-x1*(-1)",">=","0.0625"]]
#constraint = [["x1",">=","-8"],["x2",">=","-8"],["x3",">=","-8"],["x4",">=","-8"],["x5",">=","-8"],["x1","<=","8"],["x2","<=","8"],["x3","<=","8"],["x4","<=","8"],["x5","<=","8"]]
#constraint = [["x1",">=","2"],["x1","<=","8"],["x2","<=","7"],["x2",">=","3"]]
    ...
    for j in range(0, num_constraints):
        ...
        if sign == "<=":
            ...
            if float(rhs) >=0:
                ...
                if splitVar[0] == st:     #burada değişiklik yaptın!!!! >=- şaretli kısıtlar için
            ...
            print("Hata oluştu!")
        itr += 1
        print("iteration", itr, "lower", VarInterval[k, 0], "upper", VarInterval[k, 1])
