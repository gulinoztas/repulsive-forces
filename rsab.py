...
class benchmark():
    ...
    def find_upper_limit(self, sign, st, VarInterval, j, k):
        benchmark(self.num_var,self.num_const,self.lowerlimit,self.upperlimit).const()
        for i in range(0, self.num_var):
            if i == k:
                st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i], "x")
                # print("x konulduğunda", st)
            elif sign == ">=":
                st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i],
                                min((str(0.000000000000001 + (VarInterval[i, 0]))),
                                    str(abs(float(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).const()[j][2])))))
                # print("upper kısıt", st)
            else:
                st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i],
                                min((str(0.000000000000001 + (VarInterval[i, 0]))),
                                    str(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).const()[j][2])))
                # print("upper2 kısıt", st)
        st = st + "-" + str(abs(float(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).const()[j][2])))
        return st
    def find_lower_limit(self, st, VarInterval, j, k):
        for i in range(0, self.num_var):
            if i == k:
                ...
                st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i],max((str(0.000000000000001 + (VarInterval[i, 0]))), str(abs(float(benchmark(self.num_var, self.num_const, self.lowerlimit,self.upperlimit).const()[j][2])))))
        st = st + "-" + str(abs(float(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).const()[j][2])))
        return st
    ...
    def find_coefficient(self,st, k, sign):
        for i in range(0, self.num_var):
            if i == k:
                st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i], "1")
            else:
                if sign == ">=":
                    st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i], "0")
                elif st[1] == "<=":
                    st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i], "0")
                else:
                    st = st.replace(benchmark(self.num_var, self.num_const,self.lowerlimit,self.upperlimit).strVars()[i], "0")
    ...
    def adjiman(self,x,y):
        ...
        return objVal
    def determine(self):
        global VarInterval, myDataTableIntervals,Interval
        VarInterval = np.zeros((self.num_var, 2))
        myDataTableIntervals = np.array(())
        lowerlimitt = np.array(())
        upperlimitt = np.array(())
        ref = benchmark(self.num_var, self.num_const, self.lowerlimit, self.upperlimit)
...
class evaluate(benchmark):
    ...
    def modify(self,rndCategoryLow,rndCategoryUp):
        k = int(4 * random() + 1)
        IntegerVars = False
        if k == 1:
    ...
    def Goal(self,objVal,constraintSatisfiedRate,maxDeviatedConst):
        ...
        if objective == "Min":
            ...
            Goall = goalSign * goal1 * goal2 * goal3
        return Goall
    def evaluate_constraint(self,counter,CurrentInterval):
        ...
        for i in range(0,self.rand_set_size):
            ...
            for j in range(0, self.num_var):
                ...
                trainSet[i, j] = ref2.modify(rndCategoryLow,rndCategoryUp)
            ...
            if myObjVal != 1.0E+308:
                ...
                objVal = ref.dejongf1(DataTableTrain[i,2:self.num_var+2])
            ...
            if i != -1:
                ...
                if ImpYes == True:
                    ...
                    for j in range(0, self.num_var):
                        ...
                        #ref.determine()[j][3] = trainSet[i, j]
        ...
        global titles
        titles = ref.strVars()
        #DataTableTrain = pd.DataFrame(data=DataTableTrain, index=None,columns=["Order","Bias",titles[0],titles[1],"objval", "satisfied constraints", "maxdeviated", "mygoal"])
...
class improve(evaluate):
    ...
    def bestOrder(self, DataTableTrain):
        ...
        res = [i for i, j in enumerate(DataTableTrain.iloc[:, self.num_var + 5]) if j == ek]
        return res[0]
    def update(self,CurrentInterval,uni_low,uni_up):
        for k in range(0, self.num_var):
            randInterval_lower = float(uniform(uni_low, uni_up) * (holdBest[k] - float(VarInterval[k, 0])) + uniform(uni_low, uni_up) * abs(holdBest[k] - float(CurrentInterval[k][1])))
            randInterval_upper = float(uniform(uni_low, uni_up) * (float(VarInterval[k, 1]) - holdBest[k]) + uniform(uni_low, uni_up) * abs(float(CurrentInterval[k][2]) - holdBest[k]))
            if holdBest[k] - randInterval_lower - self.num_const * (1 - holdBest[self.num_var + 2]) * (holdBest[self.num_var + 3]) < 0 and (holdBest[k]) - randInterval_lower - self.num_const * (1 - holdBest[self.num_var + 2]) * (holdBest[self.num_var + 3]) > float(VarInterval[k, 0]):
                CurrentInterval[k][1] = ((holdBest[k]) - randInterval_lower - self.num_const * (1 - holdBest[self.num_var + 2]) * (holdBest[self.num_var + 3]))
            else:
                CurrentInterval[k][1] = VarInterval[k, 0]
            if (holdBest[k]) + randInterval_upper + self.num_const * (1 - holdBest[self.num_var + 2]) * (holdBest[self.num_var + 3]) > 0 and (holdBest[k]) + randInterval_upper + self.num_const * (1 - holdBest[self.num_var + 2]) * (holdBest[self.num_var + 3]) < float(VarInterval[k, 1]):
                CurrentInterval[k][2] = ((holdBest[k]) + randInterval_upper + self.num_const * (1 - holdBest[self.num_var + 2]) * (holdBest[self.num_var + 3]))
            else:
                CurrentInterval[k][2] = VarInterval[k, 1]
    ...
    def improve_solution(self,DataTableTrain,current_best_in_iteration,CurrentInterval):
        ...
        for t in range(1,self.textboxiterations+1):
            ...
            if holdBest[self.num_var+4]<= DataTableTrain.iloc[current_best_in_iteration, self.num_var + 5]:
                CurrentInterval = improve(self.num_var, self.num_const, self.lowerlimit, self.upperlimit, self.rand_set_size,self.textboxiterations).update(CurrentInterval,uni_low,uni_up)
            else:
                CurrentInterval = improve(self.num_var, self.num_const, self.lowerlimit, self.upperlimit, self.rand_set_size,self.textboxiterations).update(CurrentInterval,uni_low,uni_up)
            DataTableTrain = evaluate(self.num_var,self.num_const,self.lowerlimit,self.upperlimit,self.rand_set_size).evaluate_constraint(counter,CurrentInterval)
            print(t,CurrentInterval)
            ek = min(DataTableTrain.iloc[:, self.num_var + 5])
            res = [i for i, j in enumerate(DataTableTrain.iloc[:, self.num_var + 5]) if j == ek]
            current_best_in_iteration = res[0]
            hold = np.append(hold,holdBest[self.num_var+4])
            if t!=1:
...
def ortalama(sütun,tablo):
    ortalama_bestsofar = np.array(())
    for i in range(0,sütun):
        x=mean(tablo.iloc[:,i])
        ortalama_bestsofar=np.append(ortalama_bestsofar,x)
    return ortalama_bestsofar
def rsab(num_var,num_const,lowerlimit,upperlimit,rand_set_size,textboxiterations,run):
    bestsofar = np.array(())
    limits = np.array(())
    for p in range(1, run+1):
        counter = 1
        CurrentInterval = benchmark(num_var,num_const,lowerlimit,upperlimit).determine()
        DataTableTrain = evaluate(num_var,num_const,lowerlimit,upperlimit,rand_set_size).evaluate_constraint(counter, CurrentInterval)
    ...
    limits = limits.reshape(run, num_var*2)
    best = bestsofar.reshape(run, num_var+6)
    #bestso = pd.DataFrame(data=best, index=None,columns=["x01", "x02", "x03", "x04", "x05", "x06", "x07", "x08", "x09", "x10", "x11","x12", "x13", "x14", "x15", "x16", "x17", "x18", "x19", "x20", "x21", "x22","x23", "x24", "x25", "x26", "x27", "x28", "x29", "x30", "best order id","best objval", "constraint satisfied rate", "total error", "best goal"])
    ...
    export_excel = limitss.to_excel(r'C:\Users\Pau\Desktop\desktop\rsab-results\dejongf1limits.xlsx', index=None, header=True)
    export_excel = bestso.to_excel(r'C:\Users\Pau\Desktop\desktop\rsab-results\dejongf1.xlsx', index=None, header=True)
    ortalama_best = ortalama(num_var+6,bestso)
    ortalama_limit = ortalama(num_var*2,limitss)
    return bestso,limitss

print(rsab(3,6,[-5.12]*3,[5.12]*3,10,50,30))
